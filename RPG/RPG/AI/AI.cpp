...
void AIFactory::AddAIEntry ( AI* ai, LPCSTR name ) {
...
AI* AIFactory::CreateAI ( LPCSTR name ) {
	if ( !name || strcmp ( name, "" ) == 0 )
		return defaultAI->Clone ( );

	for ( UINT i = 0; i < dynamicAIs.size ( ); i++ ) {
		if ( strcmp ( name, dynamicAIs[i].name ) == 0 ) {
...
void UnitAI::Apply ( Object* obj, float fDeltaTime ) {
	...
	if ( characterContext ) {
		...
		obj->GetOriDev()->SetPosition ( input.m_position(0), input.m_position(1), input.m_position(2) );
...
void UnitAI::SetPosition ( hxVector3 pos ) {
	APPHANDLE->core->PhysicsDevice->GetWorld()->lock ( );
...
void UnitAI::ApplyForce ( hxVector3 vec, float fDeltaTime ) {
	APPHANDLE->core->PhysicsDevice->GetWorld ( )->lock ( );
	hkVector4 v;
	v ( 0 ) = vec.x;
	v ( 1 ) = vec.y;
...
void UnitAI::SetLinearVelocity ( hxVector3 vec, float fDeltaTime ) {
	APPHANDLE->core->PhysicsDevice->GetWorld ( )->lock ( );
	hkVector4 v;
	v ( 0 ) = vec.x;
	v ( 1 ) = vec.y;
...
void PlayerAI::Apply ( Object* obj, float fDeltaTime ) {
	hxCore* core = ResourceManager::core;
	hxCamera* cam = core->CameraManager->GetDefaultCamera ( );
	...
	if ( !bMouseHidden )
	{
		...
		lx = core->InputDevice->MouseX ( MOUSEPOS_DESKTOP, 0 );
		ly = core->InputDevice->MouseY ( MOUSEPOS_DESKTOP, 0 );

		POINT pt = { 640/2, 480/2 };
		ClientToScreen ( core->WindowsDevice->GetWindow ( ), &pt );
		SetCursorPos ( pt.x, pt.y );
	}

	int mx = core->InputDevice->MouseX ( MOUSEPOS_VIEWPORT, 0 );
	int my = core->InputDevice->MouseY ( MOUSEPOS_VIEWPORT, 0 );
	...
	hxOrientationDevice od;
	od.SetPosition ( obj->GetOriDev()->GetPosition ( ) + hxVector3 ( 0, 10.0f, 0 ) );
	od.SetAngle ( 0, 0, 0 );
	od.Yaw ( fYaw );
	od.Pitch ( fPitch );
	od.Move ( -20 );
	od.Pitch ( -fPitch / 4 );
	float fDist = hxVec3Length ( cam->GetPosition ( ) - od.GetPosition ( ) );
	float fMoveSpeed = fDist * fDist * 0.01f + 1.0f * fDeltaTime * 500.0f;
	if ( fDist > fMoveSpeed ) {
		cam->Point ( od.GetPosition ( ) );
		cam->Move ( fMoveSpeed );
	} else
		cam->SetPosition ( od.GetPosition ( ) );
	cam->SetToRotation ( &od );
	movement.fAng = fYaw;
...
void CreatureAI::Apply ( Object* obj, float fDeltaTime ) {
	Object* player = UnitManager::GetPlayer ( );
	Unit* unit = (Unit*)obj;
	if ( !attribs.target )
	{
		hxOrientationDevice* p = player->GetOriDev ( );
		if ( hxVec3LengthSq ( p->GetPosition ( ) - obj->GetOriDev()->GetPosition ( ) ) <
	...
	{
		hxOrientationDevice* p = attribs.target->GetOriDev ( );
		hxVector3 l = p->GetPosition ( ) - unit->GetOriDev()->GetPosition ( );

		float fAng;
		hxVector3 _2dLook ( l.x, 0.0f, l.z );
		float dot = hxVec3Dot ( hxVector3 ( 0, 0, 1 ), _2dLook ); //get angle between the vectors
		float u = 1.0f;
		...
		movement.fAng = HX_RADTODEG ( fAng );

		if ( hxVec3LengthSq ( l ) > attribs.fHitRange * attribs.fHitRange )
			movement.forward = 1;
		else
			movement.forward = 0;
	}

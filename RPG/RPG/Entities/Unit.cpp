...
hxVector3 GetBindingPosition ( hxOrientationDevice* dev ) {
	return hxVec3TransformCoord ( hxVector3 ( ), dev->GetBindingMatrix ( ) );
}
...
void UnitManager::Initialize ( void ) {
...
void UnitManager::DealDamage ( Unit* from, float fRange, DAMAGE_STRUCT dmg ) {
	...
	for ( UINT i = 0; i < units.size ( ); i++ ) {
		if ( units[i] != from && hxVec3LengthSq ( from->GetOriDev()->GetPosition ( ) -
			units[i]->GetOriDev()->GetPosition ( ) ) < fRangeSq ) {
			DealDamage ( from, units[i], dmg );
		}
	}
}

void UnitManager::DealDamage ( Unit* from, hxVector3 pos, float fRange, DAMAGE_STRUCT dmg ) {
	float fRangeSq = fRange * fRange;
	for ( UINT i = 0; i < units.size ( ); i++ ) {
		if ( units[i] != from && hxVec3LengthSq
				( pos - units[i]->GetOriDev()->GetPosition ( ) ) <fRangeSq ) {
			DealDamage ( from, units[i], dmg );
		}
	}
}

Unit* UnitManager::FindTarget ( hxVector2 point, float maxDist ) {
	Unit* closest = nullptr;
	int cx = 2000, cy = 2000;
	for ( UINT i = 1; i < units.size ( ); i++ ) { //unit 0 is player, ignore
		int x, y;
		units[i]->GetOriDev()->Fly ( 10 ); //origin is lower than body center, attempt to balance...
		APPHANDLE->core->Point3DToScreen2D ( 0, units[i]->GetOriDev ( ), &x, &y );
		units[i]->GetOriDev()->Fly ( -10 );
		x -= point.x;
		y -= point.y;
...
Player* UnitManager::GetPlayer ( void ) {
	return player;
}
...
EquippedItem::~EquippedItem ( void ) {
	Destroy ( );
}
void EquippedItem::Load ( void ) {
	obj = ResourceManager::AllocModel ( data.modelID[0] ).obj;
	obj->GetGBufferMaterial ( )->SetCullingMode ( CULL_NONE );
	if ( data.modelID[1] != -1 ) {
		objExtra = ResourceManager::AllocModel ( data.modelID[1] ).obj;
		objExtra->GetGBufferMaterial ( )->SetCullingMode ( CULL_NONE );
	}
}
bool EquippedItem::Update ( float fDeltaTime ) {
	if ( owner && UnitManager::UnitExists ( owner ) && obj ) {
		GetOriDev()->SetPosition ( owner->GetOriDev()->GetPosition ( ) );
		GetOriDev()->SetToRotation ( owner->GetOriDev ( ) );
		if ( data.modelID[1] != -1 ) {
			objExtra->SetPosition ( owner->GetOriDev ( )->GetPosition ( ) );
			objExtra->SetToRotation ( owner->GetOriDev ( ) );
		}
		return true;
	} else
		return false;
}
void EquippedItem::Destroy ( void ) {
	ResourceManager::ReleaseModel ( obj, data.modelID[0] );
	if ( objExtra)
		ResourceManager::ReleaseModel ( objExtra, data.modelID[1] );
	objExtra = obj = nullptr;
}
bool EquippedItem::HasModel ( void ) const {
	return obj;
}
...
void Unit::SetAngle ( float fAng ) {
	hxMatrix ml = hxRotationMatrixY ( HX_DEGTORAD ( fAng ) );
	hxVector3 l = hxVec3TransformNormal ( hxVector3 ( 0, 0, 1 ), ml );
	hxVector3 r = hxVec3TransformNormal ( hxVector3 ( 1, 0, 0 ), ml );
	obj->SetULRVectors ( hxVector3 ( 0, 1, 0 ), l, r );
}
void Unit::OnDeath ( void ) {
	bDead = true;
}
void Unit::InitAnimation ( void ) {
	SetAnimationState ( IDLE, 0, 1, SMSPEED );
}
...
void Unit::StopSpellAnimation ( void ) {
	if ( anim ) {
...
hxSkeleton* Unit::GetAnimation ( void ) const {
	return anim;
}

ComboSystem* Unit::GetComboSystem ( void ) const {
	return comboSys;
}
...
void Unit::Equip ( ITEMSLOT slot, UINT itemID ) {
	...
	if ( anim && item->HasModel ( ) ) {
		if ( data.type == 0 ) {
			hxBone* bone = anim->GetBone ( 0, boneN[slot] );
			if ( bone ) {
			...
			item->obj->AttachAnimation ( anim );
			if ( item->objExtra )
				item->objExtra->AttachAnimation ( anim );
		}
	}
}
...
void Unit::Unequip ( ITEMSLOT slot ) {
	...
	if ( anim && item && item->HasModel ( ) ) {
		if ( item->data.type == 0 ) {
			...
			item->obj->AttachAnimation ( nullptr );
			if ( item->objExtra )
				item->objExtra->AttachAnimation ( nullptr );
		}
	}

	if ( equipment.itemSlots[slot] ) {
		HX_SAFE_DELETE ( equipment.itemSlots[slot] );
	}
}
...
UnitAI* Unit::GetAI ( void ) const {
	return ai;
}

void Unit::EnableAI ( void ) {
	bAIEnabled = true;
}

void Unit::DisableAI ( void ) {
	bAIEnabled = false;
}

void Unit::DisableMotion ( void ) {
	StopMoving ( );
	ai->DisableMotion ( );
}

void Unit::EnableMotion ( void ) {
	ai->EnableMotion ( );
}
